...
exports.generateInvoicePDF1 = asyncHandler(async (req, res, next) => {
  try {
    ...
    const transaction = await Transaction.findById(transactionID);
    // .populate('customerId') // Populate customer information if needed
    ...
    const page = pdfDoc.addPage([600, 400]); // Define page size (width x height)
    ...
    page.drawText(`Name: ${transaction.customerName}`, {
      x: 50,
      y: height - 180,
      size: fontSize,
      color: fontColor,
    ...
    // Add more details from the transaction and customer as needed
